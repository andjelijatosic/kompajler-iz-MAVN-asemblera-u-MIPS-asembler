...
void LexicalAnalysis::initialize()
{
	...
	//ciscenje tokena i prethodne greske
	tokenList.clear();
	errorToken = Token();
	...
	while(true)
	{
		...
		else if (nextState == INVALID_STATE)
		{
			...
			else
			{
				...
				// lakse je preko promenljive
				int errorPosition = programBufferPosition + counter - 1;

				token.makeErrorToken(errorPosition, programBuffer);
				programBufferPosition = errorPosition + 1;
				return token;
			}
		}
		else
